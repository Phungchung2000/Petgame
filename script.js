...
window.renderBannerSlider = function() {
    ...
    let displayBanners = window.banners;
    if (displayBanners.length === 0) {
        displayBanners = [{
            img: "https://vovinam.vn/wp-content/uploads/2019/04/vovinam-banner.jpg",
            title: "TINH HOA VÕ THUẬT VIỆT NAM",
            desc: "Rèn luyện thân thể - Tu dưỡng tinh thần"
        }];
    }

    displayBanners.forEach((b, idx) => {
        const slide = document.createElement('div');
        slide.className = 'slide';
        slide.innerHTML = `<img src="${b.img}" alt="Banner"><div class="banner-content"><h2>${b.title}</h2><p>${b.desc}</p></div>`;
...
window.moveSlide = function(n) {
    const total = document.getElementById('banner-wrapper').children.length;
    bannerIndex += n;
    if (bannerIndex >= total) bannerIndex = 0;
    if (bannerIndex < 0) bannerIndex = total - 1;
    window.updateSliderPosition();
    window.startBannerAutoPlay();
};
...
window.startBannerAutoPlay = function() {
    clearInterval(bannerInterval);
    bannerInterval = setInterval(() => { window.moveSlide(1); }, 5000);
};

// --- BANNER MANAGER ---
window.openBannerManager = function() {
    if (!window.isAdmin()) return;
    document.getElementById('modal-banner-manager').style.display = 'block';
    const list = document.getElementById('banner-list-admin');
    list.innerHTML = "";
    if(window.banners.length === 0) list.innerHTML = "<p>Chưa có banner tùy chỉnh nào.</p>";
    window.banners.forEach(b => {
        const div = document.createElement('div');
        div.className = 'banner-item-admin';
        div.innerHTML = `<div style="display:flex;align-items:center;"><img src="${b.img}"><div><strong>${b.title}</strong></div></div><button class="btn-delete-banner" onclick="window.deleteBanner('${b.firebaseId}')">Xóa</button>`;
...
window.addBanner = async function() {
    ...
    const fileInput = document.getElementById('new-banner-img');
    
    if(!title || !fileInput.files[0]) { alert("Cần nhập tiêu đề và chọn ảnh!"); return; }
...
window.deleteBanner = async function(id) {
    ...
    try { await deleteDoc(doc(db, COLL_BANNERS, id)); window.openBannerManager(); } catch(e) { alert("Lỗi: " + e.message); }
};
...
window.renderAttendanceTable = function() {
    ...
    clubStudents.forEach((student, index) => {
        ...
        let statusHTML = window.canManage() ? `<label class="switch"><input type="checkbox" id="status-${student.firebaseId}" ${student.isPresent ? 'checked' : ''}><span class="slider"></span></label> <button type="button" class="btn-edit-student" onclick="window.openEditModal('${student.firebaseId}')"><i class="fas fa-pen"></i></button>` : (student.isPresent ? `<span style="color:green;font-weight:bold;">Có mặt</span>` : `<span style="color:red;font-weight:bold;">Vắng</span>`);
        let noteHTML = window.canManage() ? `<input type="text" class="note-input" id="note-${student.firebaseId}" value="${student.note || ''}" placeholder="...">` : `<span>${student.note || ''}</span>`;
        let deleteBtn = window.isAdmin() ? ` <i class="fas fa-trash" style="color: #ff4444; cursor: pointer; margin-left: 10px;" onclick="window.deleteStudent('${student.firebaseId}', '${student.name}')" title="Xóa"></i>` : '';
        ...
        tr.innerHTML = `<td style="${rowStyle}">${index + 1}</td><td style="${rowStyle}"><div style="display:flex; align-items:center;"><img src="${student.img}" class="student-avatar" style="margin-right:10px;"><div><strong>${student.name}</strong> ${roleIcon} ${isMe} ${deleteBtn}${beltHTML}${infoDisplay} ${dateInfo}</div></div></td><td style="${rowStyle}" style="white-space: nowrap;">${statusHTML}</td><td style="${rowStyle}">${noteHTML}</td>`;
        tbody.appendChild(tr);
    });
};

// --- DELETE ALL (Đã giữ lại) ---
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if(parts.length >= 2) { 
                ...
                if(!exists) {
                    ...
                    batch.set(newRef, { id: Date.now() + i, club: window.currentClub, name: name, phone: phone, dob: dob, belt: belt, img: "https://cdn-icons-png.flaticon.com/512/3135/3135715.png", role: "student", isPresent: false, note: "", lastAttendanceDate: "" });
                    count++;
                }
            }
        }
...
window.saveDailyAttendance = async function() {
    ...
    try { await batch.commit(); const safeDateId = todayStr.replace(/\//g, '-'); const logDocId = `${safeDateId}_${window.currentClub}`; const logData = { date: todayStr, timestampStr: displayDateTime, club: window.currentClub, timestamp: Date.now(), records: historyRecords }; await setDoc(doc(db, COLL_HISTORY, logDocId), logData); alert("Đã lưu thành công!"); const dp = document.getElementById('history-date-picker'); if(dp) dp.valueAsDate = new Date(); const cs = document.getElementById('history-club-select'); if(cs) cs.value = window.currentClub; window.openHistorySection(); } catch (e) { alert("Lỗi: " + e.message); }
};

window.deleteStudent = async function(firebaseId, studentName) {
    if(!window.isAdmin()) { alert("Chỉ HLV trưởng mới được xóa!"); return; }
    if(confirm(`Xóa ${studentName}?`)) { try { await deleteDoc(doc(db, COLL_STUDENTS, firebaseId)); alert("Đã xóa!"); } catch (e) { alert("Lỗi: " + e.message); } }
};
...
window.handleAddStudent = async function(e) {
    ...
    // Sửa giới hạn 5MB
    if(input.files[0]) { if(input.files[0].size > 5 * 1024 * 1024) return alert("Ảnh quá lớn (<5MB)"); img = await readFileAsBase64(input.files[0]); }
    
    await addDoc(collection(db, COLL_STUDENTS), { id: Date.now(), club: window.currentClub, name, phone, dob, belt, img, role, isPresent: false, note: "", lastAttendanceDate: "" });
...
window.saveStudentEdits = async function(e) {
    ...
    if (imgInput.files && imgInput.files[0]) { if (imgInput.files[0].size > 5 * 1024 * 1024) { alert("Ảnh quá lớn (<5MB)!"); return; } imgUrl = await readFileAsBase64(imgInput.files[0]); }
    try {
...
window.checkLoginStatus();
console.log("✅ SYSTEM UPDATE 16.0: ALL FEATURES RESTORED & FIXED");
