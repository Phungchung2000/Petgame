...
// --- QUAN TRỌNG: HÀM HIỂN THỊ TIN TỨC ---
window.renderNews = function() { 
    const list = document.getElementById('news-feed'); 
    if(!list) return;
    list.innerHTML = ""; 
    
    // Nút đăng bài (Chỉ hiện cho Admin)
    const btnCreate = document.getElementById('btn-create-post'); 
    if(btnCreate) btnCreate.style.display = window.isAdmin() ? 'block' : 'none'; 
    ...
    window.posts.forEach(post => { 
        ...
        // Tạo nút Admin (Sửa/Xóa)
        let adminActions = "";
        if (window.isAdmin()) {
            adminActions = `
                <div class="admin-actions">
                    <button class="btn-edit-post" onclick="window.editPost('${post.firebaseId}')">
                        <i class="fas fa-edit"></i> Sửa
                    </button>
                    <button class="btn-delete-post" onclick="window.deletePost('${post.firebaseId}')">
        ...
        // Lưu ý: class "news-preview" sẽ được CSS cắt gọn ảnh và nội dung
        div.innerHTML = `
            <h3>${post.title}</h3>
            <small style="color:#666;">${post.date}</small>
            <div class="news-preview">${post.content}</div>
            
            <div class="read-more-btn" onclick="window.viewPost('${post.firebaseId}')">
                Xem & Bình luận >>
            </div>
...
window.viewPost = function(firebaseId) { 
    ...
    // Ẩn nút đăng bài khi xem chi tiết
    if(document.getElementById('btn-create-post')) document.getElementById('btn-create-post').style.display = 'none'; 
    
    document.getElementById('detail-title').innerText = post.title; 
    document.getElementById('detail-date').innerText = post.date; 
    
    // Nội dung chi tiết (class article-content sẽ cho phép ảnh phóng to)
    document.getElementById('detail-content').innerHTML = post.content; 
...
window.backToNewsList = function() { 
    ...
    // Hiện lại nút đăng bài nếu là Admin
    if(document.getElementById('btn-create-post') && window.isAdmin()) {
        document.getElementById('btn-create-post').style.display = 'block'; 
    }
}
