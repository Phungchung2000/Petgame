...
window.checkLoginStatus = function() {
    ...
    if (window.currentUser) {
        ...
        // Hiện menu Lịch sử với tên phù hợp
        if (menuHistory) {
    ...
    clubStudents.forEach((student, index) => {
        ...
        let statusHTML = window.isAdmin() 
            ? `<label class="switch"><input type="checkbox" id="status-${student.firebaseId}" ${student.isPresent ? 'checked' : ''}><span class="slider"></span></label> 
               <button type="button" class="btn-edit-student" title="Sửa thông tin" onclick="window.openEditModal('${student.firebaseId}')"><i class="fas fa-pen"></i></button>`
...
// --- HÀM LƯU ĐIỂM DANH (ĐÃ NÂNG CẤP ĐỂ LƯU VÀO LỊCH SỬ) ---
window.saveDailyAttendance = async function() {
    ...
    // Tạo ngày tháng chuẩn (dd/mm/yyyy) để tránh lỗi lệch ngày
    const d = new Date();
    ...
    try {
        ...
        // 3. LƯU VÀO COLLECTION 'attendance_logs' (TRANG QUẢN TRỊ)
        const safeDateId = todayStr.replace(/\//g, '-'); // Ví dụ: 17-01-2026
        const logDocId = `${safeDateId}_${window.currentClub}`; // ID duy nhất cho mỗi CLB mỗi ngày
        ...
        alert("Đã lưu điểm danh thành công! Đang chuyển đến trang Quản trị...");
        
        // 4. TỰ ĐỘNG CHUYỂN SANG TRANG QUẢN TRỊ ĐỂ XEM KẾT QUẢ
        // Thiết lập bộ lọc cho trang quản trị trước khi mở
        const datePicker = document.getElementById('history-date-picker');
        const clubSelect = document.getElementById('history-club-select');
        if(datePicker) datePicker.valueAsDate = new Date(); // Chọn ngày hôm nay
        if(clubSelect) clubSelect.value = window.currentClub; // Chọn CLB vừa điểm danh
        
        window.openHistorySection(); // Mở trang
...
window.openHistorySection = function() {
    ...
    if (window.isAdmin()) {
        ...
        // Nếu input date chưa có giá trị thì gán ngày hôm nay
...
window.previewProfileAvatar = function(input) {
    if (input.files && input.files[0]) {
        if (input.files[0].size > 1 * 1024 * 1024) { alert("Ảnh quá lớn (<1MB)!"); return; }
        const reader = new FileReader();
...
window.saveStudentEdits = async function(e) {
    ...
    if (imgInput.files && imgInput.files[0]) { if (imgInput.files[0].size > 1 * 1024 * 1024) { alert("Ảnh quá lớn (<1MB)!"); return; } imgUrl = await readFileAsBase64(imgInput.files[0]); }
    try {
