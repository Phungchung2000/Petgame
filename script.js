...
onSnapshot(collection(db, COLL_STUDENTS), (snapshot) => {
    ...
    if (currentUser && !isAdmin()) {
        ...
        if (myRecords.length === 0) {
            alert("Tài khoản bị xóa."); window.logout(); return;
        }
...
window.renderAdminCalendar = async function() {
    ...
    for (let i = 1; i <= daysInMonth; i++) {
        ...
        dayDiv.className = className + todayClass;
        dayDiv.innerText = i;
        if (hasData) {
            dayDiv.onclick = () => showDailyRecords(dateStr);
        }
...
window.showDailyRecords = function(dateStr) {
    ...
    records.forEach(log => {
        ...
        div.innerHTML = `
            <h4>${log.club}</h4>
            <p>Sĩ số: ${log.details.length}</p>
            <p>Có mặt: ${log.details.filter(s => s.isPresent).length}</p>
            <button class="btn-view-detail" onclick="viewLogDetail('${log.id}')">Xem chi tiết >></button>
        `;
...
window.viewLogDetail = function(logId) {
    ...
    document.getElementById('detail-club-name').innerText = `Chi tiết: ${log.club} (${log.date})`;
    
    const tbody = document.getElementById('detail-student-list');
    tbody.innerHTML = "";

    log.details.forEach(student => {
        const row = document.createElement('tr');
        row.innerHTML = `
            <td>${student.name}</td>
            <td style="color: ${student.isPresent ? 'green' : 'red'}; font-weight: bold;">
                ${student.isPresent ? 'Có mặt' : 'Vắng'}
            </td>
            <td>${student.note || ''}</td>
        `;
...
// --- 8. LƯU ĐIỂM DANH (CẬP NHẬT: LƯU VÀO LỊCH SỬ) ---
window.saveDailyAttendance = async function() {
    if(!isAdmin()) { alert("Chỉ HLV mới được lưu!"); return; }
...
document.getElementById('register-form').addEventListener('submit', async function(e) {
    ...
    if (students.some(s => s.phone === phone && s.club === club)) { alert(`Số điện thoại này đã đăng ký tại CLB ${club} rồi!`); return; }
...
document.getElementById('login-form').addEventListener('submit', function(e) {
    ...
    if (matchedRecords.length > 0) { currentUser = { ...matchedRecords[0], clubs: matchedRecords.map(s => s.club), role: "student" }; loginSuccess(); } else { alert("Thông tin không đúng hoặc chưa đăng ký!"); }
});
...
function checkLoginStatus() {
    ...
    if (currentUser) {
        ...
        clubLinks.forEach(li => { const onclickText = li.getAttribute('onclick'); if (onclickText) { const clubName = onclickText.match(/'([^']+)'/)[1]; if (isAdmin() || (currentUser.clubs && currentUser.clubs.includes(clubName))) { li.style.display = 'block'; } else { li.style.display = 'none'; } } });
...
function renderAttendanceTable() {
    const tbody = document.getElementById('attendance-list'); tbody.innerHTML = ""; const clubStudents = students.filter(s => s.club === currentClub); if (clubStudents.length === 0) { document.getElementById('empty-list-msg').style.display = 'block'; return; } else { document.getElementById('empty-list-msg').style.display = 'none'; }
    clubStudents.forEach((student, index) => {
        const tr = document.createElement('tr');
        let statusHTML = isAdmin() ? `<label class="switch"><input type="checkbox" id="status-${student.firebaseId}" ${student.isPresent ? 'checked' : ''}><span class="slider"></span></label>` : (student.isPresent ? `<span style="color:green;font-weight:bold;">Có mặt</span>` : `<span style="color:red;font-weight:bold;">Vắng</span>`);
        ...
        let deleteBtn = isAdmin() ? ` <i class="fas fa-trash" style="color: #ff4444; cursor: pointer; margin-left: 10px;" onclick="deleteStudent('${student.firebaseId}', '${student.name}')" title="Xóa"></i>` : '';
        const isMe = !isAdmin() && currentUser.phone === student.phone; const rowStyle = isMe ? 'background-color: #e3f2fd; border-left: 5px solid #0055A4;' : ''; let dateInfo = student.lastAttendanceDate ? `<br><small style="color:blue">Ngày: ${student.lastAttendanceDate}</small>` : '';
        ...
        tr.innerHTML = `<td style="${rowStyle}">${index + 1}</td><td style="${rowStyle}"><img src="${student.img}" class="student-avatar"></td><td style="${rowStyle}"><strong>${student.name}</strong> ${isMe ? '(Bạn)' : ''} ${deleteBtn} ${infoDisplay} ${dateInfo}</td><td style="${rowStyle}">${statusHTML}</td><td style="${rowStyle}">${noteHTML}</td>`;
        tbody.appendChild(tr);
    });
}
window.deleteStudent = async function(firebaseId, studentName) { if(!isAdmin()) return; if(confirm(`Xóa môn sinh ${studentName}?`)) { try { await deleteDoc(doc(db, COLL_STUDENTS, firebaseId)); alert("Đã xóa!"); } catch (e) { alert("Lỗi: " + e.message); } } }
document.getElementById('add-student-form').addEventListener('submit', async function(e) {
    e.preventDefault(); const name = document.getElementById('student-name').value; const phone = document.getElementById('student-phone').value; const dob = document.getElementById('student-dob').value; const imgInput = document.getElementById('student-img');
    if (students.some(s => s.phone === phone && s.club === currentClub)) { alert("Môn sinh này đã có trong danh sách CLB này rồi!"); return; }
    let imgUrl = "https://cdn-icons-png.flaticon.com/512/3135/3135715.png"; if (imgInput.files && imgInput.files[0]) { if (imgInput.files[0].size > 500 * 1024) { alert("Ảnh quá lớn!"); return; } imgUrl = await readFileAsBase64(imgInput.files[0]); }
    const newStudent = { id: Date.now(), club: currentClub, name: name, phone: phone, dob: dob, img: imgUrl, isPresent: false, note: "", lastAttendanceDate: "" }; await addDoc(collection(db, COLL_STUDENTS), newStudent); alert(`Đã thêm ${name}!`); document.getElementById('add-student-form').reset(); window.switchTab('attendance');
});
...
window.previewProfileAvatar = function(input) { if (input.files && input.files[0]) { const reader = new FileReader(); reader.onload = function(e) { document.getElementById('profile-img-preview').src = e.target.result; } reader.readAsDataURL(input.files[0]); } }
window.saveProfile = async function(e) { e.preventDefault(); if (!currentUser || !confirm("Lưu thay đổi?")) return; const newName = document.getElementById('profile-name').value; const newDob = document.getElementById('profile-dob').value; const newImg = document.getElementById('profile-img-preview').src; const myRecords = students.filter(s => s.phone === currentUser.phone); const batch = writeBatch(db); myRecords.forEach(rec => { const docRef = doc(db, COLL_STUDENTS, rec.firebaseId); batch.update(docRef, { name: newName, dob: newDob, img: newImg }); }); try { await batch.commit(); currentUser.name = newName; currentUser.dob = newDob; currentUser.img = newImg; localStorage.setItem('vovinamCurrentUser', JSON.stringify(currentUser)); alert("Đã cập nhật hồ sơ!"); window.disableEditMode(); checkLoginStatus(); } catch(e) { alert("Lỗi: " + e.message); } }
...
function renderNews() { const list = document.getElementById('news-feed'); list.innerHTML = ""; const btnCreate = document.getElementById('btn-create-post'); if(btnCreate) btnCreate.style.display = isAdmin() ? 'block' : 'none'; posts.forEach(post => { const div = document.createElement('div'); div.className = 'news-item'; let adminActions = isAdmin() ? `<div class="admin-actions"><button class="btn-edit-post" onclick="editPost('${post.firebaseId}')"><i class="fas fa-edit"></i> Sửa</button><button class="btn-delete-post" onclick="deletePost('${post.firebaseId}')"><i class="fas fa-trash-alt"></i> Xóa</button></div>` : ""; div.innerHTML = `<h3>${post.title}</h3><small style="color:#666;">${post.date}</small><div class="news-preview">${post.content}</div><div class="read-more-btn" onclick="viewPost('${post.firebaseId}')">Xem & Bình luận >></div>${adminActions}`; list.appendChild(div); }); }
window.viewPost = function(firebaseId) { const post = posts.find(p => p.firebaseId === firebaseId); if (!post) return; currentViewingPostId = firebaseId; document.getElementById('news-list-view').style.display = 'none'; document.getElementById('news-detail-view').style.display = 'block'; if(document.getElementById('btn-create-post')) document.getElementById('btn-create-post').style.display = 'none'; document.getElementById('detail-title').innerText = post.title; document.getElementById('detail-date').innerText = post.date; document.getElementById('detail-content').innerHTML = post.content; renderComments(post); window.scrollTo(0, 0); }
window.backToNewsList = function() { document.getElementById('news-detail-view').style.display = 'none'; document.getElementById('news-list-view').style.display = 'block'; currentViewingPostId = null; if(document.getElementById('btn-create-post') && isAdmin()) document.getElementById('btn-create-post').style.display = 'block'; }
function renderComments(post) { const list = document.getElementById('comment-list'); list.innerHTML = ""; if (!post.comments) post.comments = []; if (post.comments.length === 0) { list.innerHTML = "<p style='color:#777;font-style:italic;'>Chưa có bình luận.</p>"; return; } post.comments.forEach((cmt, idx) => { const div = document.createElement('div'); div.className = 'comment-item'; let role = cmt.isAdminComment ? "admin-role" : ""; let name = cmt.isAdminComment ? "HLV - Quản Trị" : cmt.userName; let delBtn = isAdmin() ? `<button class="btn-delete-cmt" onclick="deleteComment('${post.firebaseId}', ${idx})"><i class="fas fa-trash"></i></button>` : ""; div.innerHTML = `<div class="comment-author ${role}">${name} <span class="comment-date">(${cmt.date})</span></div><div class="comment-text">${cmt.text}</div>${delBtn}`; list.appendChild(div); }); }
window.submitComment = async function(e) { e.preventDefault(); if (!currentUser) { alert("Vui lòng đăng nhập!"); window.showSection('login'); return; } const text = document.getElementById('comment-input').value.trim(); if (!text) return; const post = posts.find(p => p.firebaseId === currentViewingPostId); if (!post) return; const newComment = { userId: currentUser.phone, userName: currentUser.name, text: text, date: new Date().toLocaleString(), isAdminComment: isAdmin() }; let updatedComments = post.comments || []; updatedComments.push(newComment); await updateDoc(doc(db, COLL_POSTS, currentViewingPostId), { comments: updatedComments }); document.getElementById('comment-input').value = ""; }
window.deleteComment = async function(firebaseId, commentIndex) { if (!confirm("Xóa bình luận?")) return; const post = posts.find(p => p.firebaseId === firebaseId); if(!post) return; let updatedComments = post.comments.filter((_, idx) => idx !== commentIndex); await updateDoc(doc(db, COLL_POSTS, firebaseId), { comments: updatedComments }); }
