...
// --- 6. CHỨC NĂNG SỬA THÔNG TIN MÔN SINH (MỚI) ---
...
window.saveDailyAttendance = async function() {
    ...
    try { await batch.commit(); alert(`Đã lưu trạng thái cho ${count} môn sinh!`); } catch (e) { console.error(e); alert("Lỗi: " + e.message); }
}
window.deleteStudent = async function(firebaseId, studentName) {
    if(!isAdmin()) return;
    if(confirm(`Xóa vĩnh viễn ${studentName}?`)) { try { await deleteDoc(doc(db, COLL_STUDENTS, firebaseId)); alert("Đã xóa!"); } catch (e) { alert("Lỗi: " + e.message); } }
}
...
function renderComments(post) { const list = document.getElementById('comment-list'); list.innerHTML = ""; if (!post.comments) post.comments = []; if (post.comments.length === 0) { list.innerHTML = "<p style='color:#777;font-style:italic;'>Chưa có bình luận.</p>"; return; } post.comments.forEach((cmt, idx) => { const div = document.createElement('div'); div.className = `comment-item ${cmt.isAdminComment ? 'is-admin' : 'is-student'}`; let roleBadge = cmt.isAdminComment ? `<span class="comment-role-badge">Quản trị viên</span>` : ""; let authorName = cmt.isAdminComment ? "Huấn Luyện Viên" : cmt.userName; let delBtn = isAdmin() ? `<button class="btn-delete-cmt" onclick="deleteComment('${post.firebaseId}', ${idx})"><i class="fas fa-trash"></i></button>` : ""; div.innerHTML = `<div class="comment-header"><span class="comment-author-name">${authorName} ${roleBadge}</span><span class="comment-date">${cmt.date}</span></div><div class="comment-text">${cmt.text}</div>${delBtn}`; list.appendChild(div); }); }
window.submitComment = async function() { if (!currentUser) { alert("Vui lòng đăng nhập!"); window.showSection('login'); return; } const content = document.getElementById('comment-input-rich').innerHTML.trim(); if (!content) return; const post = posts.find(p => p.firebaseId === currentViewingPostId); if (!post) return; const newComment = { userId: currentUser.phone, userName: currentUser.name, text: content, date: new Date().toLocaleString(), isAdminComment: isAdmin() }; let updatedComments = post.comments || []; updatedComments.push(newComment); await updateDoc(doc(db, COLL_POSTS, currentViewingPostId), { comments: updatedComments }); document.getElementById('comment-input-rich').innerHTML = ""; }
